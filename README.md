...
```bash
# Unit + system tests (~4s)
npm test

# Java CDK build (~45s)
./mvnw clean verify

# Local E2E behaviour tests
...
npm run auth    # Mock OAuth2 server
...
Deployment is handled by GitHub Actions. Push to a branch to deploy:

```bash
git checkout -b claude/my-feature
git push origin claude/my-feature
# GitHub Actions deploys to ci account
```
